...
const NewPost = () => {
  ...
  const onSubmit=async()=>{
    if(!bodyRef.current && !file){
      Alert.alert('Post','Please add some content or media');
      return;
    }
            ...
            <View style={styles.file}>
             {getFileType(file) === 'video' ? (
              ...
              <Image
                source={{ uri: getFileUri(file) }}
                resizeMode="cover"
                style={{ width: '100%', height: 200 }} // Explicit dimensions
              />
          )}
          <Pressable style={styles.closeIcon} onPress={()=>setFile(null)}>
            <Icon name="delete" size={25} color="white" style={styles.closeIcon}/>
          </Pressable>
            </View>
